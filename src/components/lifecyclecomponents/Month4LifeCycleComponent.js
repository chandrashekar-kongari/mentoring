...
const Month4LifeCycleComponent = () => {
  ...
  useEffect(()=>{
    setLifeCycle(li)
    const foundMonth4Data = li.find(item => item.id === 'month4');
    setMonth4Data(foundMonth4Data);
    setAgenda(foundMonth4Data.agenda)
    setInsights(foundMonth4Data.insights)
    setTasks(foundMonth4Data.tasks)
  ...
  const [resources,setResources]=useState('')
    ...
    const handleDateChange=async(newval)=>{
      ...
      try {
        const response = await axios.post(`${endpoint}/updatelifecycle`,obj);
            ...
            if (response.status === 200) {
              ...
              if(user==null){
                ...
                const foundMonth4Data = response.data.user.lifeCycle.find(item => item.id === 'month4');
                setMonth4Data(foundMonth4Data);
                setAgenda(foundMonth4Data.agenda)
                setInsights(foundMonth4Data.insights)
                setTasks(foundMonth4Data.tasks)
...
export default Month4LifeCycleComponent